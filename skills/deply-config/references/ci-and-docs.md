...
```bash
python -m pip install "deply==1.1.2"
...
```bash
pipx run --spec "deply==1.1.2" deply validate --config=deply.yaml
pipx run --spec "deply==1.1.2" deply analyze --parallel --config=deply.yaml
```

`uv`:

```bash
uvx --from "deply==1.1.2" deply validate --config=deply.yaml
uvx --from "deply==1.1.2" deply analyze --parallel --config=deply.yaml
```

`poetry`:

```bash
poetry add --group dev "deply==1.1.2"
...
- name: Validate Deply config
  run: uvx --from "deply==1.1.2" deply validate --config=deply.yaml

- name: Check architecture
  run: uvx --from "deply==1.1.2" deply analyze --parallel --report-format=github-actions --config=deply.yaml
```
